package c_statement;

import java.util.Scanner;

public class ALoveTest {

	private static final String String = null;

	public static void main(String[] args) {

		// && m == "YES" && m == "Yes" && m == "YEs" && m == "yES" && m == "yeS"
				// && m == "No" && m == "No" && m == "nO"
				System.out.println("나의 연애 유형 테스트를 시작하겠습니다. \n답변은 항상 YES or NO로 하세요.");
				
				Scanner s = new Scanner(System.in);
				System.out.println("나는 금사빠다 *금방 사랑에 빠진다");
				String A = "A: 서로에 대한 신뢰감이 깊고, 존중해주는 어른스러운 연애를 하는 타입!";
				String B = "B: 구속을 하는 것도 , 받는 것도 싫은 자유로운 연애를 하는 타입!";
				String C = "C: 이것은 의리인가 사랑인가... 친구같이 편안한 연애를 하는 타입!";
				String D = "D: 무조건 잘해주고, 무조건 맞춰주는 다 퍼주는 연애를 하는 타입!";
				String q2 = "연애할 때 끌려다니는 타입이다";
				String q3 = "데이트 코스는 미리 짜는게 편하다";
				String q4 = "감정기복이 크지 않다";
				String q5 = "감정 표현에 솔직한 편이다";
				String q6 = "활동적인 데이트가 좋다";
				String q7 = "연락이 없어도 믿고 기다리는 편이다";
				String q8 = "이성친구는 존재할 수 없다";
				String q9 = "아무것도 아닌 일에 쉽게 섭섭함이 쌓인다";
				
				
				String a1 = s.nextLine();
				
				if(a1.equalsIgnoreCase("YES")) {
					System.out.println(q2);
				 	String a2 = s.nextLine();
				 	if (a2.equalsIgnoreCase("YES")) {
				 		System.out.println(q5);
				 		String a5 = s.nextLine();
				 		if (a5.equalsIgnoreCase("YES")) {
				 			System.out.println(q8);
				 			String a8 = s.nextLine();
				 			if (a8.equalsIgnoreCase("YES")) {
				 				System.out.println(q9);
				 				String a9 = s.nextLine();
				 				if (a9.equalsIgnoreCase("YES")) {
				 					System.out.println(D);			
				 				} else {
				 					System.out.println(C);
				 				} 
				 			} else {
				 				System.out.println(B);
				 			}
				 		} else {
				 			System.out.println(q6);
				 			String a6 = s.nextLine();
							if (a6.equalsIgnoreCase("yes")) {
								System.out.println(q8);
								String a8 = s.nextLine();
								if (a8.equalsIgnoreCase("YES")) {
									System.out.println(q9);
									String a9 = s.nextLine();
									if (a9.equalsIgnoreCase("YES")) {
										System.out.println(D);			
										} else {
											System.out.println(C);
										} 
									} else {
										System.out.println(B);
									}
				 			} else {
				 				System.out.println(q9);
								String a9 = s.nextLine();
								if (a9.equalsIgnoreCase("YES")) {
									System.out.println(D);			
									} else {
										System.out.println(C);
									} 
								}
				 			}
				 	} else {
				 		System.out.println(q3);
				 		String a3 = s.nextLine();
				 		if (a3.equalsIgnoreCase("yes")) {
				 			System.out.println(q6);
				 			String a6 = s.nextLine();
				 			if (a6.equalsIgnoreCase("yes")) {
				 				System.out.println(q8);
				 				String a8 = s.nextLine();
				 				if (a8.equalsIgnoreCase("YES")) {
				 					System.out.println(q9);
				 					String a9 = s.nextLine();
				 					if (a9.equalsIgnoreCase("YES")) {
				 						System.out.println(D);			
				 						} else {
				 							System.out.println(C);
				 						} 
				 				} else {
				 					System.out.println(B);
				 				}
				 			} else {
				 				System.out.println(q9);
				 				String a9 = s.nextLine();
				 				if (a9.equalsIgnoreCase("YES")) {
				 					System.out.println(D);			
				 				} else {
				 					System.out.println(C);
				 				} 
				 			}
				 			
				 		} else {
				 			System.out.println(q5);
				 			String a5 = s.nextLine();
				 			if (a5.equalsIgnoreCase("yes")) {
				 				System.out.println(q8);
				 				String a8 = s.nextLine();
				 				if (a8.equalsIgnoreCase("YES")) {
				 					System.out.println(q9);
				 					String a9 = s.nextLine();
									if (a9.equalsIgnoreCase("YES")) {
										System.out.println(D);			
										} else {
											System.out.println(C);
										} 
				 				} else {
				 					System.out.println(B);
								}
				 			} else {
				 				System.out.println(q6);
					 			String a6 = s.nextLine();
					 			if (a6.equalsIgnoreCase("yes")) {
					 				System.out.println(q8);
					 				String a8 = s.nextLine();
					 				if (a8.equalsIgnoreCase("YES")) {
					 					System.out.println(q9);
					 					String a9 = s.nextLine();
					 					if (a9.equalsIgnoreCase("YES")) {
					 						System.out.println(D);			
					 						} else {
					 							System.out.println(C);
					 						} 
					 				} else {
					 					System.out.println(B);
					 				}
					 			} else {
					 				System.out.println(q9);
				 					String a9 = s.nextLine();
				 					if (a9.equalsIgnoreCase("YES")) {
				 						System.out.println(D);			
				 						} else {
				 							System.out.println(C);
				 						} 
					 			}
				 			}
				 		}
				 	}	
		} else {
			System.out.println(q4);
			String a4 = s.nextLine();
			if (a4.equalsIgnoreCase("yes")) {
				System.out.println(q7);
				String a7 = s.nextLine();
				if (a7.equalsIgnoreCase("yes")) {
					System.out.println(A);
				} else {
					System.out.println(q8);
					String a8 = s.nextLine();
					if (a8.equalsIgnoreCase("YES")) {
						System.out.println(q9);
						String a9 = s.nextLine();
						if (a9.equalsIgnoreCase("YES")) {
							System.out.println(D);			
						} else {
							System.out.println(C);
						} 
					} else {
						System.out.println(B);
					}
				}
				
			} else {
				System.out.println(q5);
	 			String a5 = s.nextLine();
	 			if (a5.equalsIgnoreCase("yes")) {
	 				System.out.println(q8);
	 				String a8 = s.nextLine();
	 				if (a8.equalsIgnoreCase("YES")) {
	 					System.out.println(q9);
	 					String a9 = s.nextLine();
						if (a9.equalsIgnoreCase("YES")) {
							System.out.println(D);			
							} else {
								System.out.println(C);
							} 
	 				} else {
	 					System.out.println(B);
					}
	 			} else {
	 				System.out.println(q6);
		 			String a6 = s.nextLine();
		 			if (a6.equalsIgnoreCase("yes")) {
		 				System.out.println(q8);
		 				String a8 = s.nextLine();
		 				if (a8.equalsIgnoreCase("YES")) {
		 					System.out.println(q9);
		 					String a9 = s.nextLine();
		 					if (a9.equalsIgnoreCase("YES")) {
		 						System.out.println(D);			
		 						} else {
		 							System.out.println(C);
		 						} 
		 				} else {
		 					System.out.println(B);
		 				}
		 			} else {
		 				System.out.println(q9);
	 					String a9 = s.nextLine();
	 					if (a9.equalsIgnoreCase("YES")) {
	 						System.out.println(D);			
	 						} else {
	 							System.out.println(C);
	 						} 
		 			}
	 			}
			}
		}
	}
}
